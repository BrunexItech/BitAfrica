...
import { 
  ...
  Cpu, Cloud, Database, Rocket, Brain
} from 'lucide-react';
...
const AboutUs = () => {
  ...
  const achievements = [
    { icon: <Award className="h-4 w-4" />, title: "AWS Partner", year: "2024", gradient: "from-amber-400 to-yellow-500" },
    { icon: <TrendingUp className="h-4 w-4" />, title: "500% Growth", year: "2023", gradient: "from-emerald-400 to-green-500" },
  ...
  return (
    <div className="relative overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 py-20 px-6 md:px-12">
      
      {/* Dynamic gradient orbs - Dark mode optimized */}
      <motion.div 
        className="absolute top-20 right-10 w-72 h-72 bg-gradient-to-r from-cyan-900/30 to-blue-800/20 rounded-full blur-3xl"
        animate={{ 
          scale: [1, 1.2, 1],
          opacity: [0.2, 0.3, 0.2]
        }}
      ...
      />
      <motion.div 
        className="absolute -bottom-20 -left-20 w-80 h-80 bg-gradient-to-r from-purple-900/25 to-pink-800/15 rounded-full blur-3xl"
        animate={{ 
          x: [0, 20, 0],
          y: [0, -20, 0]
        }}
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="text-center mb-14">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/10 shadow-sm mb-6"
          >
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-12">
          ...
          <div className="space-y-6">
            <motion.div
              ...
              transition={{ duration: 0.6 }}
              whileHover={{ y: -5 }}
              className="bg-gray-800/70 backdrop-blur-xl rounded-2xl p-6 border border-gray-700/50 shadow-xl hover:shadow-2xl transition-all duration-300"
            >
              <div className="flex items-center gap-3 mb-6">
                <div className="p-2.5 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 shadow-lg">
                  <Sparkles className="h-5 w-5 text-white" />
                </div>
                <h3 className="text-xl font-bold text-gray-100">Our Genesis</h3>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-gray-700/30">
                <h4 className="text-sm font-bold mb-4 text-gray-100">Enterprise Milestones</h4>
                <div className="grid grid-cols-2 gap-3">
                  {achievements.map((item, idx) => (
                    <motion.div
                      ...
                      className="group p-3 rounded-xl bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-gray-700/50 hover:border-gray-600/50 hover:shadow-lg transition-all duration-300"
                    >
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${item.gradient} bg-gradient-to-br`}>
                          {React.cloneElement(item.icon, { className: "h-4 w-4 text-white" })}
                        </div>
                        <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {teamStats.map((stat, idx) => (
                <motion.div
                  ...
                  transition={{ duration: 0.4, delay: idx * 0.1 }}
                  whileHover={{ y: -4 }}
                  className="group bg-gray-800/60 backdrop-blur-sm rounded-xl p-4 border border-gray-700/40 hover:border-gray-600/50 shadow-lg hover:shadow-xl transition-all duration-300"
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-xl font-black bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">
                      {stat.value}
                    </div>
                    <div className="p-1.5 rounded-lg bg-gradient-to-br from-blue-900/30 to-cyan-900/20 group-hover:scale-110 transition-transform duration-300">
                      {React.cloneElement(stat.icon, { className: "h-4 w-4 text-blue-400" })}
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-bold text-gray-100">Our Mission</h3>
              </div>
              ...
              <div className="space-y-3">
                {values.map((value, idx) => (
                  <motion.div
                    ...
                    transition={{ duration: 0.4, delay: idx * 0.1 }}
                    whileHover={{ x: 4 }}
                    className="group flex items-start gap-3 p-3 rounded-xl bg-gradient-to-r from-gray-800/40 via-gray-800/30 to-gray-800/20 border border-gray-700/40 hover:border-gray-600/50 shadow-sm hover:shadow-lg transition-all duration-300"
                  >
                    <div 
                      className={`p-2.5 rounded-lg bg-gradient-to-br ${value.color} group-hover:scale-105 transition-transform duration-300`}
                    >
                      {React.cloneElement(value.icon, { className: "h-4 w-4 text-white" })}
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-gray-100 mb-1">{value.title}</h4>
            ...
            <motion.div
              ...
              transition={{ duration: 0.6 }}
              whileHover={{ y: -5 }}
              className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-2xl p-6 border border-gray-700/50 shadow-xl hover:shadow-2xl transition-all duration-300"
            >
              <h3 className="text-lg font-bold mb-4 text-gray-100">AI Stack</h3>
              <div className="grid grid-cols-2 gap-3">
                {techFocus.map((tech, idx) => (
                  <motion.div
                    ...
                    className="group flex items-center gap-3 p-3 rounded-lg bg-gray-800/50 backdrop-blur-sm border border-gray-700/40 hover:border-gray-600/50 hover:shadow-lg transition-all duration-300"
                  >
                    <div className={`p-2 rounded-md ${tech.color} group-hover:scale-105 transition-transform duration-300`}>
                      {React.cloneElement(tech.icon, { className: "h-4 w-4 text-white" })}
                    </div>
                    <span className="text-sm font-semibold text-gray-200">{tech.name}</span>
                  </motion.div>
                ))}
              </div>
              <p className="text-xs text-gray-400 mt-4 pt-3 border-t border-gray-700/30">
        ...
        >
          <div className="relative p-0.5 rounded-2xl bg-gradient-to-r from-blue-600 via-cyan-500 to-purple-600">
            <div className="bg-gray-800/95 backdrop-blur-xl rounded-xl p-8 border border-gray-700/50">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.98 }}
                  className="px-6 py-3 rounded-xl border-2 border-gray-600 hover:border-blue-500 bg-gray-800/80 hover:shadow-lg transition-all duration-300"
                >
