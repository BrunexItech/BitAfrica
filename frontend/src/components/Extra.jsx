          ...
          <nav className="hidden lg:flex items-center space-x-1">
            {navItems.map((item) => (
              <div key={item.name} className="relative group">
                ...
                {item.submenu && (
                  <div className="absolute left-1/2 -translate-x-1/2 mt-2 w-60 bg-[#1f2937]/95 backdrop-blur-xl rounded-xl shadow-2xl border border-blue-500/20 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform -translate-y-2 group-hover:translate-y-0 z-50">
                    <div className="p-2">
                      {item.submenu.map((sub, index) => (
                        <Link
                          key={sub.name}
                          to={sub.href}
                          className="flex items-center px-4 py-2.5 text-blue-100/80 hover:text-white hover:bg-gradient-to-r hover:from-blue-500/10 hover:to-cyan-500/10 rounded-lg transition-all duration-200 group/item"
            ...
            {/* Solutions Dropdown - PROFESSIONAL SPACING */}
            <div 
        ...
        {isMenuOpen && (
          ...
          >
            <div className="space-y-0 px-2">
              ...
              {/* Rest of mobile navigation items - COMPACT SPACING */}
              {navItems.slice(1).map((item, index) => (
                <div key={item.name} className="overflow-hidden">
                  ...
                  {item.submenu && openMobileItems[item.name] && (
                    <div className="ml-6 mt-1 space-y-0 bg-gradient-to-b from-blue-500/10 to-cyan-500/10 rounded-lg p-1 border border-blue-500/20 animate-slideDown">
                      {item.submenu.map((sub, subIndex) => (
                        <Link
                          key={sub.name}
                          to={sub.href}
                          className="flex items-center px-3 py-2.5 text-sm text-blue-200/80 hover:text-white hover:bg-gradient-to-r hover:from-blue-500/10 hover:to-cyan-500/10 rounded-lg transition-all duration-200 group/sub"
                          onClick={() => setIsMenuOpen(false)}
                        >
                          <div className="h-1.5 w-1.5 rounded-full bg-cyan-400 opacity-0 group-hover/sub:opacity-100 mr-2 transition-opacity"></div>
              ...
              {/* Mobile Solutions Accordion - WITH 6 ITEMS */}
