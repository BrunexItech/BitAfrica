  ...
  return (
    <div className="flex select-none">
      ...
      <main className={`flex-1 p-4 md:p-6 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'} select-none`}>
        ...
        {/* Courses Section - PROFESSIONAL VERTICAL LAYOUT */}
        <div className="mb-8 select-none">
            ...
            <div className="space-y-1 select-none">
              {filteredCourses.map((course) => {
                ...
                return (
                  <div
                    key={course.id}
                    className={`border ${darkMode ? 'border-gray-700' : 'border-gray-200'} ${isExpanded ? 'rounded-lg' : 'rounded-lg'} overflow-hidden transition-all duration-300 select-none`}
                  >
                    {/* Course Title Row - Click to Expand */}
                    <button
                      onClick={() => toggleCourseExpansion(course.id)}
                      className={`w-full text-left px-6 py-4 flex items-center justify-between ${darkMode ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'} transition-colors border-b ${isExpanded ? (darkMode ? 'border-gray-700' : 'border-gray-200') : 'border-transparent'}`}
                    >
                      <div className="flex items-center flex-1 min-w-0">
                        <div className={`p-2 rounded-md ${darkMode ? 'bg-gray-700' : 'bg-gray-100'} mr-3 border ${courseColor.border}`}>
                          {courseColor.icon}
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-1">
                            ...
                            <span className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'} flex items-center`}>
                      ...
                      <div className="flex items-center ml-4">
                        ...
                        <span className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'} w-10 text-right`}>
                          {Math.round(progressPercentage)}%
                        </span>
                        {isExpanded ? (
                          <ChevronUp className="ml-3 text-gray-400" size={20} />
                        ) : (
                          <ChevronDown className="ml-3 text-gray-400" size={20} />
                        )}
                      </div>
                    </button>
                    ...
                    {/* Expanded Content */}
                    {isExpanded && (
                      <div className="px-6 py-4 bg-gray-50/50 dark:bg-gray-800/30 border-t dark:border-gray-700">
                        {/* Course Description */}
                            ...
                            {Array.from({ length: course.total_modules || 0 }).map((_, index) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`w-full text-left px-3 py-2 rounded-lg flex items-center justify-between ${darkMode ? 'hover:bg-gray-700/50' : 'hover:bg-gray-100'} transition-colors`}
                                >
                                  <div className="flex items-center">
